...
class EnvCallableMcpChromeBridge:
    ...
    def list_network_requests(
        ...
        try:
            ...
            if {"resource_types", "page_size", "include_preserved_requests", "page_idx"}.issubset(names):
                return fn(**kwargs_snake)
            if {"resource_types", "page_size", "include_preserved_requests"}.issubset(names):
                kwargs_snake.pop("page_idx", None)
                return fn(**kwargs_snake)
            if {"resourceTypes", "pageSize", "includePreservedRequests", "pageIdx"}.issubset(names):
                return fn(**kwargs_camel)
            if {"resourceTypes", "pageSize", "includePreservedRequests"}.issubset(names):
                kwargs_camel.pop("pageIdx", None)
                return fn(**kwargs_camel)
        ...
        try:
            ...
            try:
                return fn(resource_types, page_size, include_preserved_requests, page_idx)
            except TypeError:
