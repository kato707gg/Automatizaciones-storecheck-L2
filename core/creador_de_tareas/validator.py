...
class TaskIRValidator:
    ...
    def validate(self, ir: TaskIR) -> ValidationResult:
        ...
        for edge in normalized.edges:
            ...
            if edge.condition_type in {"stage", "answer"} and edge.condition_value in {"0", "1"}:
                if parent.capture_data_type != "6":
                    issues.append(
                        ValidationIssue(
                            code="YESNO_STAGE_REQUIRES_PARENT_6",
                            message=(
                                f"Condición {edge.condition_type} con valor {edge.condition_value} requiere padre tipo 6 (Si/No). "
