...
def build_playbook(ir: TaskIR) -> list[PlaybookAction]:
    ...
    index = 2
    # Precompute edges grouped by child to interleave attach actions after child creation/update
    edges_by_child: dict[str, list[Any]] = {}
    for edge in ir.edges:
        edges_by_child.setdefault(edge.child_block_id, []).append(edge)
    ...
    for block in ir.blocks:
        ...
        # After creating and updating a block, immediately attach any edges where this block is the child.
        for edge in edges_by_child.get(block.block_id, []):
            actions.append(
                PlaybookAction(
                    step_id=f"S{index:02d}",
    ...
    # Note: edges already interleaved above.
