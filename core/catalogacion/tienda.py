...
def catalogacion_por_tienda(ruta_matriz, ruta_base, carpeta_trabajo):
    ...
    Encuentra productos que NO están completos para su formato 
    (no aparecen en todas las tiendas del formato)
    
    Args:
        ruta_matriz: Ruta al archivo MATRIZ DE CATALOGACIÓN en la subcarpeta
        ruta_base: Ruta a la carpeta principal (donde está layout_place_scope_188865)
        carpeta_trabajo: Ruta a la subcarpeta de trabajo
    """
    print("\n" + "="*50)
    print("--- Catalogación Por Tienda ---")
    print("="*50)
    ...
    for archivo in os.listdir(ruta_base):
        if "layout_place_scope_188865" in archivo.lower() and archivo.endswith(".xlsx"):
    ...
    try:
        ...
        # Cache de la fila de IDs de productos para no releerla en cada iteración
        ...
        dict_conteo_formatos = defaultdict(int)  # Cuenta cuántas tiendas tiene cada formato
        dict_conteo_productos_por_formato = defaultdict(int)  # Cuenta en cuántas tiendas aparece cada producto de cada formato
        
        print("  Paso 1: Contando formatos y productos por formato...")
        
        # Paso 1: Contar formatos y productos por formato (datos ya en memoria)
        ...
        print(f"  Combinaciones formato-producto: {len(dict_conteo_productos_por_formato)}")
        
        # Preparar datos de salida
        datos_salida = []
        
        print("  Paso 2: Generando datos de catalogación por tienda...")
